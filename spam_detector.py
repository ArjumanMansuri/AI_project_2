...
def create_model(vocabulary, ham_dictionary, spam_dictionary):
    ...
    for i, word in enumerate(vocabulary):
        ...
        f.write(str(i + 1) + '  ' + word + '  ' + str(freq_in_ham) + '  ' + str(
            "{:.8f}".format(float(c_p_in_ham))) + '  ' + str(freq_in_spam) + '  ' + str(
            "{:.8f}".format(float(c_p_in_spam))) + '\n')
...
for file_path in file_list:
    ...
    with open(file_path, encoding='latin-1') as infile:
        ...
        f.write(str(str(temp_counter) + " " + str(file_name) + " " + str(predicted_classification) + " " + str(
            score_log_ham) + " " + str(score_log_spam) + " " + str(correct_classification) + " " + str(label) + "\n"))
...
confusion_matrix(values=np.array([[true_positive, false_positive],
                                  [false_negative, true_negative]]), class_name=['Ham', 'Spam'],
                 title="Confusion matrix for Ham Class")

confusion_matrix(values=np.array([[true_negative, false_negative],
                                  [false_positive, true_positive]]), class_name=['Spam', 'Ham'],
                 title="Confusion matrix for Spam class")
...
accuracy_model = (true_positive + true_negative) / (
...
col_label = ("Class", "Accuracy", "Precision", "Recall", "F1-measure")
axs[0].axis('tight')
axs[0].axis('off')
axs[1].axis("off")
the_table = axs[0].table(cellText=data, colLabels=col_label, loc='center')
